...
class MaskTransformerTrainer:
    ...
    def save(self, file_name, ep, total_it):
        ...
        state = {
            ...
            'ep': ep,
            'total_it': total_it,
            # Save RNG states so interrupted jobs can continue with the same
            # sampling/masking behavior when the checkpoint contains them.
        ...
        print(f"[Checkpoint] Saved epoch {ep} → {file_name}")
    ...
    def resume(self, model_dir):
        ...
        print(f"[Checkpoint] Resumed from epoch {checkpoint['ep']} → {model_dir}")
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        auto_resume = getattr(self.opt, 'auto_resume', False)
        if self.opt.is_continue or auto_resume:
            model_dir = pjoin(self.opt.model_dir, 'latest.tar')  # TODO
            if os.path.isfile(model_dir):
                epoch, it = self.resume(model_dir)
                print("Load model epoch:%d iterations:%d"%(epoch, it))
            elif self.opt.is_continue:
                raise FileNotFoundError(f"Requested resume checkpoint does not exist: {model_dir}")
            else:
                print(f"[Checkpoint Load] No latest checkpoint found at {model_dir}; starting from epoch 1")

        start_time = time.time()
        train_loader_provider = train_loader if hasattr(train_loader, 'get_loader') else None
        active_train_loader = train_loader_provider.get_loader(epoch + 1) if train_loader_provider else train_loader
        total_iters = self.opt.max_epoch * len(active_train_loader)
        print(f'Total Epochs: {self.opt.max_epoch}, Initial Estimated Total Iters: {total_iters}')
        print('Iters Per Epoch, Training: %04d, Validation: %03d' % (len(active_train_loader), len(val_loader)))
        ...
        while epoch < self.opt.max_epoch:
            ...
            active_train_loader = train_loader_provider.get_loader(epoch_number) if train_loader_provider else train_loader
            if train_loader_provider:
                print(train_loader_provider.describe(epoch_number))
            print(f"[Epoch] {epoch_number}/{self.opt.max_epoch}")
            print(f"[Dataset Size] {len(active_train_loader.dataset)} samples, {len(active_train_loader)} batches")

            self.t2m_transformer.train()
            self.vq_model.eval()

            for i, batch in enumerate(active_train_loader):
                it += 1
