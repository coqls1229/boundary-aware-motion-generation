...
class MaskTransformer(nn.Module):
    def __init__(self, code_dim, cond_mode, latent_dim=256, ff_size=1024, num_layers=8,
                 num_heads=4, dropout=0.1, text_dim=768, cond_drop_prob=0.1,
        ...
        if self.cond_mode == 'text':
            print('Loading BERT...')
            self.bert_model, self.bert_tokenizer = self.load_and_freeze_bert()
            self.text_dim = self.bert_model.config.hidden_size  # 보통 768
    ...
    def encode_text(self, raw_text):
        ...
        # 방법 1: CLS token 사용 (가장 일반적)
        text_feat = outputs.last_hidden_state[:, 0, :]  # (bs, hidden_dim)

        # 방법 2 (대안): mean pooling
    ...
    def forward(self, ids, y, m_lens):
        ...
        non_pad_mask = lengths_to_mask(m_lens, ntokens).to(device) #(b, n)
    ...
    def generate(self,
        ...
        m_lens = m_lens.to(device)
        padding_mask = ~lengths_to_mask(m_lens, seq_len).to(device)
        # print(padding_mask.shape, )

        # Start from all tokens being masked
        ids = torch.where(padding_mask, self.pad_id, self.mask_id).to(device)
        scores = torch.where(padding_mask, torch.tensor(1e5, device=device), torch.tensor(0., device=device))
