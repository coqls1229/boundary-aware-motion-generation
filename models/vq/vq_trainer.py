...
class RVQTokenizerTrainer_resume:
    def __init__(self, args, vq_model):
        self.opt = args
        self.vq_model = vq_model
        self.device = args.device
    ...
    # --------------------------------------------------
    # RESUME (epoch 없음 → it만 복원)
    # --------------------------------------------------
    def resume(self, model_dir):
        print(f"\n[RESUME] loading checkpoint from: {model_dir}")

        checkpoint = torch.load(model_dir, map_location=self.device)

        print("[RESUME] checkpoint keys:", checkpoint.keys())
        ...
        print(f"[RESUME] restored total_it: {it}\n")
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval=None):
        ...
        if self.opt.is_continue:
            ...
            print(f"[TRAIN] resumed total_it: {it}\n")
        ...
        # --------------------------------------------------
        # TRAIN LOOP (iteration 기반)
        # --------------------------------------------------
        while it < self.opt.max_epoch * len(train_loader):

            epoch = it // len(train_loader)

            self.vq_model.train()

            print(f"[TRAIN LOOP] epoch: {epoch}, it: {it}")
            print(f"[TRAIN LOOP] is_continue: {self.opt.is_continue}")
            ...
            val_loss, val_loss_rec, val_loss_vel, val_loss_commit, val_perplexity = [], [], [], [], []
            ...
            print("Validation Loss: %.5f Reconstruction: %.5f, Velocity: %.5f, Commit: %.5f" %
                  (sum(val_loss)/len(val_loss),
                   ...
                   sum(val_loss_vel)/len(val_loss_vel),
                   sum(val_loss_commit)/len(val_loss_commit)))
            ...
            if epoch % self.opt.eval_every_e == 0:
                data = torch.cat([self.motions[:4], self.pred_motion[:4]], dim=0).detach().cpu().numpy()
                save_dir = pjoin(self.opt.eval_dir, f"E{epoch:04d}")
                os.makedirs(save_dir, exist_ok=True)
                plot_eval(data, save_dir)
