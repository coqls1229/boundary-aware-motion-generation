...
def evaluation_mask_transformer_test_plus_res(val_loader, vq_model, res_model, trans, repeat_id, eval_wrapper,
    ...
    print("GT NaN:", np.isnan(motion_annotation_np).any())
    print("PRED NaN:", np.isnan(motion_pred_np).any())

    motion_annotation_np = np.nan_to_num(motion_annotation_np)
    motion_pred_np = np.nan_to_num(motion_pred_np)
